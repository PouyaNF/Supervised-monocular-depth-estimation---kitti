...
def densenet161(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('pretrained_model/encoder/densenet161-8d451a50.pth')
        for key in list(checkpoint.keys()):
            checkpoint[key.replace('norm.1', 'norm-1').replace('norm.2', 'norm-2').replace('conv.1', 'conv-1').replace('conv.2', 'conv-2')] = checkpoint.pop(key)
        model.load_state_dict(checkpoint)
        print('pretrained encoder loaded - densenet161')
    return model
...
class _DenseLayer(nn.Sequential):
    ...
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super(_DenseLayer, self).__init__()
        self.add_module('norm-1', nn.BatchNorm2d(num_input_features)),                        #  norm.1
        self.add_module('relu-1', nn.ReLU(inplace=True)),
        self.add_module('conv-1', nn.Conv2d(num_input_features, bn_size *
                                            growth_rate, kernel_size=1, stride=1, bias=False)),
        self.add_module('norm-2', nn.BatchNorm2d(bn_size * growth_rate)),
